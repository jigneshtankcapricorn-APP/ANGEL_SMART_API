...
if not st.session_state.connected:
    st.warning("👈 Please **login from the sidebar** to run Signals.", icon="🔑")
    st.stop()
...
if run_btn:
    ...
    st.session_state.pop('signals_meta', None)
...
if run_btn or 'signals_data' in st.session_state:
    if 'signals_data' not in st.session_state:
        ...
        src = "Watchlist" if use_wl and st.session_state.get('watchlist_data') is not None else "All 500 stocks"
        ...
        for i, (_, row) in enumerate(universe.iterrows()):
            ...
            vol     = ohlc[-1]['volume']
            avg_vol = sum(c['volume'] for c in ohlc[-21:-1]) / 20 if len(ohlc) >= 21 else row['ref_avg_vol_20d']
        ...
        st.session_state.signals_meta = {
            ...
            'high_vol': high_vol, 'total': len(universe)
        }

    df  = st.session_state.signals_data
